...
if [ ! -e "$GENOME_PATH/Csites.csv" ]; then echo "$GENOME_PATH/Csites.csv not found"; exit 1; fi
if [ ! -e "$GENOME_PATH/Gsites.csv" ]; then echo "$GENOME_PATH/Gsites.csv not found"; exit 1; fi
...
echo `date`" - Exporting plus strand CpG sites to $PROJECT.plus.CpG.gz";
...
echo `date`" - Exporting plus strand CpH sites to $PROJECT.plus.C.gz";
$SAMTOOLS_PATH pileup "$PROJECT".plus.bam | cut -f 1,2,5 | awk -v genomeDir="$GENOME_PATH" -v strand="+" '
function nextCpG() {
    if((getline tmp < (genomeDir"/Csites.csv"))==0) exit
    split(tmp, tmp2, ",")
    chr=tmp2[1]
    chrNo=chrTable[chr]
    pos=tmp2[2]
...
}' | gzip -c > "$PROJECT".plus.C.gz;

            echo `date`" - Exporting minus strand CpG sites to $PROJECT.minus.CpG.gz";
...
echo `date`" - Exporting minus strand DpG sites to $PROJECT.minus.G.gz";
$SAMTOOLS_PATH pileup "$PROJECT".minus.bam | cut -f 1,2,5 | awk -v genomeDir="$GENOME_PATH" -v strand="-" '
function nextCpG() {
    if((getline tmp < (genomeDir"/Gsites.csv"))==0) exit
    split(tmp, tmp2, ",")
    chr=tmp2[1]
    chrNo=chrTable[chr]
    pos=tmp2[2]
